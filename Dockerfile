...
#RUN echo "deb [check-valid-until=no] https://snapshot.debian.org/archive/debian/20230131T034648Z/ unstable main" > /etc/apt/sources.list
#RUN echo "deb-src [check-valid-until=no] https://snapshot.debian.org/archive/debian/20230131T034648Z/ unstable main" >> /etc/apt/sources.list
##
#########
...
WORKDIR /shim-review
RUN git checkout debian-shim-15.7-debian-12
WORKDIR /
...
WORKDIR /shim
RUN git checkout debian/15.7-1
